          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-1">
            {Object.entries(data.removalReasons).map(([reason, count]) => (
              <div key={reason} className="flex justify-between items-baseline">
                ...
                <dd className="font-medium tabular-nums text-xs">{count}</dd>
              </div>
            ))}
          </dl>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {Object.entries(data.qualityFlags).map(([flag, count]) => (
              <span
                key={flag}
                className="inline-flex items-center rounded-md bg-amber-50 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 ring-1 ring-inset ring-amber-600/20 dark:bg-amber-950/30 dark:text-amber-300"
              >
...
function Metric({ label, value, tone = "default" }: { label: string; value: number; tone?: "default" | "success" | "warning" }) {
  return (
    <div className="rounded-lg border border-border/40 bg-muted/20 px-2.5 py-2">
      ...
      <div className={`text-base font-semibold tabular-nums mt-0.5 font-mono ${
          ...
          <div className="space-y-3.5">
            ...
            <div className="space-y-2.5">
              {result.perSpecies.map((species) => (
                <div key={species.scientificName} className="rounded-lg border border-border/40 p-2.5 bg-muted/15">
                  <h4 className="mb-2 font-medium text-[13px] italic">{species.scientificName}</h4>
